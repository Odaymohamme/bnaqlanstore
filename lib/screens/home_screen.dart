...
class _HomeScreenState extends State<HomeScreen>
  ...
  // new: controller for scroll pagination
  ...
  void initState() {
    ...
    // listen internet changes and show overlay
  ...
  /// 🗂 تحميل الفئات والعروض والمنتجات (أول دفعة)
  Future<void> _loadData() async {
    try {
      final results = await Future.wait([
        _firestore.collection('categories').get(),
        _firestore.collection('special_offers').get(),
        // تحميل أول دفعة من items (مع محاولة استخدام created_at)
        _firestore.collection('items').orderBy('created_at', descending: true).limit(_pageSize).get(),
      ]);
      ...
      // fallback: if orderBy('created_at') failed (field missing), try orderBy(documentId)
      debugPrint('⚠️ تحميل أولي بcreated_at فشل: $e — محاولة بديل documentId');
      try {
        final catSnap = await _firestore.collection('categories').get();
        final offerSnap = await _firestore.collection('special_offers').get();
        final itemSnap = await _firestore.collection('items').orderBy(FieldPath.documentId).limit(_pageSize).get();

        _categories = catSnap.docs
            .map((d) => Category.fromFirestore(d.data() as Map<String, dynamic>, d.id))
            .toList();

        _offers = offerSnap.docs
            .map((d) => SpecialOffer.fromFirestore(d.data() as Map<String, dynamic>, d.id))
            .toList();
        ...
        if (itemSnap.docs.isNotEmpty) _lastItemDoc = itemSnap.docs.last;
        if (itemSnap.docs.length < _pageSize) _hasMoreItems = false;

        if (mounted) setState(() {});
      } catch (e2) {
        debugPrint('❌ فشل البديل أيضًا: $e2');
      }
    }
  }

  /// تحميل المزيد عند التمرير (pagination) — يحاول استخدام created_at ثم documentId كبديل
  Future<void> _loadMoreItems() async {
    if (_isLoadingMore || !_hasMoreItems) return;
    if (_lastItemDoc == null) return;

    _isLoadingMore = true;
    try {
      // حاول أولًا باستخدام created_at ordering
      Query<Map<String, dynamic>> query = _firestore.collection('items')
          .orderBy('created_at', descending: true)
          .startAfterDocument(_lastItemDoc!)
          .limit(_pageSize);
      ...
      try {
        ...
        // إذا فشل (مثلاً created_at غير موجود) حاول ب documentId
        debugPrint('⚠️ تحميل المزيد باستخدام created_at فشل: $e — محاولة documentId');
        final q2 = _firestore.collection('items')
            .orderBy(FieldPath.documentId)
            .startAfterDocument(_lastItemDoc!)
            .limit(_pageSize);
        snap = await q2.get() as QuerySnapshot<Map<String, dynamic>>;
      }
      ...
      if (snap.docs.isEmpty) {
        ...
        _lastItemDoc = snap.docs.last;
        if (newItems.length < _pageSize) _hasMoreItems = false;
      }
    } catch (e) {
      debugPrint('❌ خطأ في تحميل المزيد: $e');
      // لا نُحدد _hasMoreItems هنا لأن قد يكون خطأ مؤقت
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        // الإنترنت overlay (مربع في منتصف الشاشة، بدون أزرار)
        if (!_hasInternet)
          Container(
