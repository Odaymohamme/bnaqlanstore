...
class _ProductDetailState extends State<ProductDetail> {
  List<Unit> _units = [];
  Unit? _selectedUnit; // المستخدم لم يختر وحدة بعد
  int _qty = 1;

  bool _loading = true;
  bool _adding = false;

  late double _itemPrice; // سعر الصنف الأساسي
  ...
  Future<void> _fetchUnits() async {
    ...
    setState(() {
      _units = units;
      _selectedUnit = null; // نبدأ بالسعر الافتراضي
      _loading = false;
    });
  }
        ...
        body: _loading
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  "السعر: ${_currentPrice.toStringAsFixed(2)} ر.س",
                  style: const TextStyle(
                ...
                if (_units.isNotEmpty)
                  Column(
                    ...
                    children: [
                      ...
                      DropdownButton<Unit>(
                        value: _selectedUnit,
                        hint: const Text("الوحدة الافتراضية (من سعر الصنف)"),
                        isExpanded: true,
                        items: _units.map((u) {
                          return DropdownMenuItem<Unit>(
                            value: u,
                            child: Text(
                                "${u.name} - ${u.price.toStringAsFixed(2)} ر.س"),
                          );
                        }).toList(),
                        onChanged: (val) {
